...
def setup_wizard():
    ...
    if summarizer_choice == "a":
        ...
        model = input("   Enter model name (default: facebook/bart-large-cnn): ").strip()
        if model:
...
def serve_command(args):
    ...
    try:
        ...
        client_secret = config_manager.resolve_secret_ref(config.oauth.client_secret_ref)
    ...
    if not credentials:
        ...
        if config.summarizer.engine == "remote":
            api_key = config_manager.resolve_secret_ref(config.summarizer.remote_api_key_ref)
