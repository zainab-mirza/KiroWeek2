...
class EmailPreprocessor:
    ...
    def clean_email(self, raw_email: RawEmail) -> CleanedEmail:
        ...
        # Handle empty body
        if not body.strip():
            logger.warning(f"Email {raw_email.message_id} has empty body after cleaning")
    ...
    def remove_quoted_replies(self, text: str) -> str:
        ...
        for line in lines:
            ...
            is_quote_header = any(re.search(pattern, line, re.IGNORECASE | re.MULTILINE) 
        ...
        for i, line in enumerate(lines):
            ...
            if i > len(lines) * 0.6:  # Only check last 40% of email
                ...
                if re.search(r'\b\d{3}[-.]?\d{3}[-.]?\d{4}\b', line):
                    ...
                    contact_indicators = sum(1 for l in next_lines 
                                           if re.search(r'@|www\.|http|phone|mobile|office', l, re.IGNORECASE))
