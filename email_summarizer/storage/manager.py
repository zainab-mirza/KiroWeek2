...
class StorageManager:
    ...
    def __init__(self, config: StorageConfig):
        ...
        self.db_path = self.summaries_dir / "index.db" if config.use_sqlite_index else None
    ...
    def list_summaries(self, limit: Optional[int] = None, offset: int = 0) -> List[EmailSummary]:
        """List all summaries.
    ...
    def save_feedback(self, message_id: str, feedback: Feedback) -> None:
        ...
        if self.db_path:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (message_id, feedback.rating, feedback.comment, feedback.created_at.isoformat()))
    ...
    def _get_file_path_from_index(self, message_id: str) -> Optional[str]:
        ...
        cursor.execute("SELECT file_path FROM summaries WHERE message_id = ?", (message_id,))
