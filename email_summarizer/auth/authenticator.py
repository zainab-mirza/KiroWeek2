...
class GmailAuthenticator(OAuthAuthenticator):
    ...
    def __init__(
        self, config: OAuthConfig, client_id: str, client_secret: str, token_file: Path
    ):
...
class OutlookAuthenticator(OAuthAuthenticator):
    ...
    def get_authorization_url(self) -> str:
        ...
        auth_url = app.get_authorization_request_url(
            scopes=self.config.scopes, redirect_uri=self.config.redirect_uri
        )
        return auth_url
    ...
    def refresh_token(self, credentials: Credentials) -> Credentials:
        ...
        result = app.acquire_token_by_refresh_token(
            credentials.refresh_token, scopes=self.config.scopes
        )

        if "error" in result:
            raise ValueError(
                f"Token refresh error: {result.get('error_description', result['error'])}"
            )
